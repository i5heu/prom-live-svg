...
Ultra-high-performance Go HTTP service that compiles Prometheus metrics into static SVG charts every 15 seconds. Designed for CDN caching with low JS overhead.
...
No `go.mod`, build scripts, or CI exist yet. Standard Go toolchain commands expected once code lands.
